...
```
Chú ý rằng kiểu dữ liệu của collection (`urType`) khi khai báo phải là tên lớp đối tượng, như là [[3. OOP#Wrapper class|wrapper class]], [[2. Cú pháp cơ bản#Chuỗi ký tự `String`|String]] hoặc các lớp collection.
...
Cả `List` và `Set` đều có thể được duyệt bằng [[2. Cú pháp cơ bản#Dùng for-each|for-each]]. `Map` được duyệt gián tiếp thông qua `.keySet()`, `.values()`, hay `.entrySet()`.

`List` có thể được duyệt bằng [[2. Cú pháp cơ bản#Dùng for|for]] vì nó có phương thức `.get()` để lấy phần tử.
...
`List` là [[2. Cú pháp cơ bản#Mảng (Array)|mảng]] động. Có 2 class implement đến là `ArrayList` và `LinkedList`.
