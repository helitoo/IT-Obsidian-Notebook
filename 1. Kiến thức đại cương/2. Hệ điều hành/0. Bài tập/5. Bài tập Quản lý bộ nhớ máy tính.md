...
>[!note] Đổi đơn vị khối lượng dữ liệu
>- 1 B = $8$ bits.
>- 1 KB = $2^{10}$ B.
>- 1 MB = $2^{20}$ B.
>- 1 GB = $2^{30}$ B.
...
>[!note] Bảng đổi đơn vị thời gian
>- 1 ms = $10^{-3}$ s.
>- 1 ns = $10^{-9}$ s.
>- 1 ns = $10^{-3}$ ms.
...
- Ta có 1 KB = $2^{10}$ B, nên page offset = $10$ bit = `0000000100`.
...
- Kích thước 1 page: $4.2^{10}=2^o\Rightarrow o=12$.
- Vậy số entries: $2^{n-o}=2^{32-12}=2^{20}$.
...
---
Cho một hệ thống có kích thước frame $1024$ Bytes. Một process có kích thước $4096$ Bytes được nạp vào $4$ frame. Biết địa chỉ luận lý $520,3456,1564,2716$ lần lượt được chuyển đổi thành địa chỉ vật lý $2568,4636,1692,5524$. **Vẽ bảng phân trang?**

Ta có:

Kích thước frame: $2^o=1024\Rightarrow o=10$.

Xác định page number và frame number:

| Logical address | Page number | Physical address | Frame number |
| --------------- | ----------- | ---------------- | ------------ |
| `520`           | `0` = `0`   | `2568`           | `10` = `2`   |
| `3456`          | `11` = `3`  | `4636`           | `100` = `4`  |
| `1564`          | `1` = `1`   | `1692`           | `1` = `1`    |
| `2716`          | `10` = `2`  | `5524`           | `101` = `5`  |
...
---
Một không gian địa chỉ ảo có $n=256$ trang, mỗi trang dài $4096$ Bytes và được ánh xạ vào bộ nhớ có $64$ khung. Nếu mỗi entry của page table có $2$ Bytes thì **kích thước bảng khung trang là?**

Đáp án: $256\times2=512$.
