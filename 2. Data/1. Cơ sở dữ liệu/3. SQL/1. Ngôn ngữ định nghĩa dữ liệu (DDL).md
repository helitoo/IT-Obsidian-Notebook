...
| MSSQL                                                                                                                             | Oracle                                                                                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Không nhất thiết có dấu `;` sau mỗi lệnh.                                                                                         | Phải có dấu `;` s                                                                                                                                         |
| Mỗi server có thể có nhiều database.<br><br>Cần:<br>- `CREATE DATABASE db`: Tạo database `db`.<br>- `USE db`: Dùng database `db`. | Mỗi server chỉ có 1 database.<br><br>Cần:<br>- Tạo user.<br>- Cấp toàn quyền (*dành cho học tập*) `GRANT DBA TO usr;`.<br>- Đăng nhập `CONNECT usr/psw;`. |

```sql
CREATE USER usr
IDENTIFIED BY psw  -- Đặt password
...
>- Phải ép kiểu, `TO_DAY(s, 'dd-mm-yyyy')`, `TO_CHAR(v)`, `TO_NUMBER(v)`,...
