...
**Vấn đề global state**: Nếu ứng dụng chỉ có một vài state đơn giản thì bạn có thể cài đặt `useState` và các hook cần thiết ngay tại function component đó. Thế nhưng, khi ứng dụng càng lớn thì:
...
**State management là quản lý các state**.
- *Global state management* đặt ra các **kho chứa** các tính toàn cục.
- Các component có thể dùng trực tiếp các state trong kho chứa mà không cần *prop drilling*.
- Hệ thống các component này được gọi là *Compound components patterns*.
...
- **`.Provider`**: Là component dùng để bao bọc các component được phép dùng các biến và hàm trong context.
- **`.Consumer`**: Là component được `.Provider` bao bọc.
