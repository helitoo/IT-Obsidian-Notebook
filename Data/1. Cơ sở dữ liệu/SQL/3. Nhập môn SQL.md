...
3. Các phép toán quan hệ
    ...
    5. Phép kết (JOIN)
    6. Phép hợp, giao, hội (UNION, UNION ALL, INTERSECT, EXCEPT)
```
...
>- Không nhất thiết cần đếm số lượng mới dùng `GROUP BY`, có thể dùng `COUNT (DISTINCT ...)`. Tương tự với các aggreeate function khác.
>- `HAVING` chỉ dùng cho các nhóm sau khi `GROUP BY`, tức là dùng `HAVING` chung với các agreeate function.

>[!tip] Để lấy giá trị nhỏ nhất, lớn nhất
>- Dùng `GROUP BY` và `MIN`, `MAX`.
>- Dùng `SELECT TOP 1` và `ORDER BY`.
...
>[!note]
>Trong nhiều trường hợp, các phép `UNION`, `INTERSECT` và `EXCEPT` có thể thay bằng `WHERE EXISTS/NOT EXISTS`.
