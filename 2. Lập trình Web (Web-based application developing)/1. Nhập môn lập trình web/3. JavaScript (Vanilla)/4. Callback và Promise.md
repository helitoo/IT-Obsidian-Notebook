...
JS là một ngôn ngữ lập trình đơn luồng, có nghĩa là chỉ có thể có một điều có thể xảy ra tại một không đồng bộ (Asynchronous). Giả sử chúng ta đang muốn lấy dữ liệu một cách không đồng bộ bằng cách sử dụng [[4. Callback và Promise#Callback function|callback]] như sau:
...
- **Async function**: Là một hàm luôn luôn trả về một **Promise**.
- **Await**: Là từ khóa dùng để tạm dừng việc thực thi các câu lệnh tiếp theo cho đến khi một *Promise được xử lý xong* (trả về kết quả hoặc lỗi).

**Mối quan hệ bắt buộc giữa `async` và `await`**:
- **Cơ chế tạm dừng**: Khi interpreter gặp `await`, nó cần biết rằng toàn bộ hàm đó đang hoạt động ở chế độ bất đồng bộ để nó có thể đợi mà không làm treo toàn bộ chương trình.
- **Giá trị trả về**: Vì hàm chứa `await` phải tạm dừng, nó không thể trả về giá trị ngay lập tức. Do đó, hàm đó bắt buộc phải được đánh dấu là `async` để JavaScript đảm bảo kết quả trả về của hàm đó luôn là một Promise.
