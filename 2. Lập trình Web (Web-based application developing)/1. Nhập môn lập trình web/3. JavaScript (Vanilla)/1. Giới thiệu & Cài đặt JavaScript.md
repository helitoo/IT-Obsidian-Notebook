...
- Đối với server, JS chạy trên môi trường [[1. Node.js]].
...
Mã JS có thể được viết trong các file `.js` riêng và nhúng vào trang `html` thông qua thẻ `<script>`. Thẻ này có 2 thuộc tính quan trọng:

---
**Dùng thuộc tính `src`**: Khi script không có module:
```html
...
---
**Thuộc tính `type="module"`**: Khi script có module:
