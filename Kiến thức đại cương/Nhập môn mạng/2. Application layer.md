...
3. Hệ thống email và Giao thức SIMP, IMAP
...
| Thuật toán định thời      | **FCFS** (First come first serve): Gói nào tới trước thì được xử lý trước. | **Robin Round: Multiplexing** (đa luồng). Mỗi luồng có một thời gian hoạt động cố định và xen kẽ. | **QUIC**: Mỗi stream là một TCP connection riêng biệt.<br>       |
...
Do HTTP có tính *stateless* nên cần có cookies. **Cookies** được dùng để tạo trạng thái cho người dùng, VD *lịch sử truy cập trang web, lịch sử mua hàng,...*.
...
# Hệ thống email và Giao thức SIMP, IMAP
