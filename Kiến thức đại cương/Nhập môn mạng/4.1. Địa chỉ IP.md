...
- Số mạng con tối đa có thể chia: $\boxed{\max{(n)}=2^{b_b}}$.
- Số host tối đa trong mạng: $\boxed{\max{(m)}=2^{b_h-b_b}-2}$ (*trừ 1 cho địa chỉ mạng và 1 cho địa chỉ broadcast*).
- Số bit còn trống, có thể mượn: $\boxed{\max{(b_b)}\leq b_h}$.
...
**NAT**: Là một kỹ thuật dùng để chuyển đổi từ *địa chỉ IP và số port* của các host trong mạng nội bộ (*private IP*) sang phần còn lại của Internet (*tức là thay đổi source address - SA*). Toàn bộ các gói tin truyền từ mạng nội bộ sẽ mang một địa chỉ IP duy nhất.

**Cơ chế: Sử dụng bảng NAT translation table**: Chứa các cặp địa chỉ WAN và LAN. Địa chỉ LAN là địa chỉ IP cục bộ và WAN là địa chỉ NAT.
