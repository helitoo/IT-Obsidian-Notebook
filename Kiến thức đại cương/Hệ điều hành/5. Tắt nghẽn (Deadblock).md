...
- $\text{Available}[i]=k$: Tài nguyên $i$ có sẵn $k$ instance.
- $\text{Max}[i,j]=k$: Process $i$ cần tối đa $k$ instance của tài nguyên $j$.
- $\text{Allocation}[i,j]=k$: Process $i$ đã được cấp $k$ instance của tài nguyên $j$.
- $\text{Need}[i,j]=k$ (Request): Process $i$ cần thêm $k$ instance của
