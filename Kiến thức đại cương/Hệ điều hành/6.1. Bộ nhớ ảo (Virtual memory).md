...
Bộ nhớ ảo là một kỹ thuật cho phép **xử lý 1 process không được nạp toàn bộ vào bộ nhớ vật lý**.

**Ưu điểm**:
- Số lượng process trong bộ nhớ nhiều hơn.
- Một process có thể thực thi ngay cả khi kích thước của nó lớn hơn bộ nhớ thực.
- Giảm nhẹ công việc của developer.
...
- **Phân trang theo yêu cầu (Demand Paging)**:
	...
	- Khi có một tham chiếu đến một trang mà không có trong bộ nhớ chính (valid bit) thì phần cứng sẽ gây ra một ngắt (*page-fault trap*) kích hoạt **page-fault service routine (PFSR)** của OS.
	- **Các bước của PFSR**:
		...
		2. Tìm frame trống (*trong bước này thì các process khác vẫn có thể được cấp CPU để thực thi*):
			...
			2. Nếu không tìm thấy (*Lỗi trang, Page fault*), dùng thuật toán thay trang nhớ để thay 1 trang (*victim page*) cho process này. Một số giải thuật thay trang:
				1. **FRFS**: Thay thế trang được dùng sớm nhất.
				2. **OPT**: Thay thế trang dự định sẽ dùng lâu nhất hoặc không dùng nữa.
				3. **LRU**: Thay thế trang chưa được dùng lâu nhất.
